...
function FileIcon({ type }: { type: SharedFile["type"] }) {
  ...
  const colors: Record<SharedFile["type"], string> = {
    pdf:"#ef4444", image:"#a78bfa", audio:"#34d399", other:"#9CA3AF",
  }
  return (
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        .cip-root {
          ...
          border-left: 1px solid var(--border-subtle, #1E2736);
        ...
        .cip-back {
          ...
          color: var(--text-muted, #4B5563); padding: 5px;
          border-radius: 7px; display: flex; transition: all .15s;
        }
        .cip-back:hover { background:var(--bg-elevated,#10151F); color:var(--text-primary,#E2E8F0); }
        ...
        .cip-av-dot { position:absolute; bottom:2px; right:2px; width:13px; height:13px; border-radius:50%; background:var(--success,#4ade80); border:3px solid var(--bg-surface,#0D1118); }
        .cip-name { font-family:'Bricolage Grotesque',sans-serif; font-size:18px; font-weight:800; letter-spacing:-.5px; margin-bottom:5px; }
        .cip-sub { font-size:12px; color:var(--text-muted,#4B5563); line-height:1.5; margin-bottom:16px; }
        ...
        .ca-icon { width:42px; height:42px; border-radius:50%; background:var(--bg-elevated,#10151F); border:1px solid var(--border-subtle,#1E2736); display:flex; align-items:center; justify-content:center; transition:all .15s; color:var(--accent,#E8B84B); }
        .ca-btn:hover .ca-icon { background:var(--accent-dim,#E8B84B20); border-color:var(--accent-border,#E8B84B40); }
        .ca-label { font-size:10px; color:var(--text-muted,#4B5563); }

        /* Body scrollable */
        .cip-body { flex:1; overflow-y:auto; }
        .cip-body::-webkit-scrollbar { width:3px; }
        .cip-body::-webkit-scrollbar-thumb { background:var(--border-subtle,#1E2736); border-radius:3px; }

        /* Section */
        .cip-section { padding:14px 18px; border-bottom:1px solid var(--border-subtle,#1E2736); }
        .cip-section-title { font-size:10px; color:var(--text-muted,#4B5563); letter-spacing:1px; text-transform:uppercase; font-weight:500; margin-bottom:10px; }

        /* Toggle notifs */
        .notif-row { display:flex; align-items:center; justify-content:space-between; }
        .notif-label { font-size:13px; color:var(--text-primary,#E2E8F0); }
        ...
        /* Tabs */
        .cip-tabs { display:flex; background:var(--bg-elevated,#10151F); border-radius:8px; padding:2px; margin:0 18px 12px; }
        .cip-tab { flex:1; padding:6px; border-radius:6px; border:none; cursor:pointer; font-size:11px; font-weight:500; font-family:'DM Sans',sans-serif; transition:all .2s; background:transparent; color:var(--text-muted,#4B5563); }
        .cip-tab.on { background:var(--bg-surface,#0D1118); color:var(--text-primary,#E2E8F0); }

        /* Members */
        .member-item { display:flex; align-items:center; gap:10px; padding:9px 0; border-bottom:1px solid var(--border-subtle,#1E2736); }
        ...
        .m-dot { position:absolute; bottom:0; right:0; width:8px; height:8px; border-radius:50%; background:var(--success,#4ade80); border:2px solid var(--bg-surface,#0D1118); }
        .m-info { flex:1; min-width:0; }
        .m-name { font-size:12px; font-weight:500; color:var(--text-primary,#E2E8F0); display:flex; align-items:center; gap:5px; }
        .m-role { font-size:9px; background:var(--accent-dim,#E8B84B20); color:var(--accent,#E8B84B); padding:1px 6px; border-radius:4px; font-weight:600; }
        .m-role.me-badge { background:var(--info-dim,#60a5fa15); color:var(--info,#60a5fa); }
        .m-actions { display:flex; gap:3px; }
        .m-action { background:none; border:none; cursor:pointer; color:var(--text-muted,#4B5563); padding:4px; border-radius:5px; display:flex; transition:all .15s; }
        .m-action:hover { background:var(--bg-elevated,#10151F); color:var(--text-primary,#E2E8F0); }
        .m-action.danger:hover { color:var(--danger,#ef4444); background:var(--danger-dim,#ef444415); }

        /* Files */
        .file-item { display:flex; align-items:center; gap:10px; padding:9px 0; border-bottom:1px solid var(--border-subtle,#1E2736); cursor:pointer; transition:opacity .15s; }
        .file-item:last-child { border-bottom:none; }
        .file-item:hover { opacity:.8; }
        .f-icon { width:34px; height:34px; border-radius:8px; background:var(--bg-elevated,#10151F); display:flex; align-items:center; justify-content:center; flex-shrink:0; }
        .f-info { flex:1; min-width:0; }
        .f-name { font-size:12px; font-weight:500; color:var(--text-primary,#E2E8F0); overflow:hidden; text-overflow:ellipsis; white-space:nowrap; margin-bottom:2px; }
        .f-meta { font-size:10px; color:var(--text-muted,#4B5563); }
        .f-dl { background:none; border:none; cursor:pointer; color:var(--text-muted,#4B5563); padding:4px; border-radius:5px; display:flex; transition:color .15s; flex-shrink:0; }
        .f-dl:hover { color:var(--accent,#E8B84B); }

        /* Danger zone */
        .danger-item { display:flex; align-items:center; gap:10px; padding:11px 0; border-bottom:1px solid var(--border-subtle,#1E2736); cursor:pointer; transition:opacity .15s; }
        ...
        .danger-sub { font-size:10px; margin-top:1px; color:var(--text-muted,#4B5563); }
      `}</style>
        ...
        <div className="cip-body">
          ...
          <div className="cip-hero">
            ...
            <div className="cip-actions">
              <button className="ca-btn" onClick={() => router.push(`/chats/${conv.id}`)} aria-label="Message">
              ...
              <button className="ca-btn" onClick={() => router.push(`/calls/new?contact=${conv.id}&type=audio`)} aria-label="Audio">
                <div className="ca-icon">
              ...
              <button className="ca-btn" onClick={() => router.push(`/calls/new?contact=${conv.id}&type=video`)} aria-label="Vidéo">
                <div className="ca-icon">
          ...
          <div className="cip-section">
            <div className="notif-row">
              ...
              <button
                className="tgl"
                style={{ background: muteNotifs ? "var(--accent,#E8B84B)" : "var(--border-default,#2a3444)" }}
              ...
              >
                <div className="tgl-knob" style={{
                  ...
                  background: muteNotifs ? "var(--accent-text,#080C14)" : "var(--text-muted,#4B5563)",
                }} />
              </button>
            </div>
          </div>
              ...
              {conv.isGroup && isAdmin && (
                <button
                  style={{
                    ...
                    background:"var(--bg-elevated,#10151F)", border:"1px dashed var(--border-default,#2a3444)",
                    borderRadius:9, padding:"9px", fontSize:12, color:"var(--text-muted,#4B5563)",
                  ...
                  onMouseEnter={e => { e.currentTarget.style.borderColor="var(--accent-border,#E8B84B40)"; e.currentTarget.style.color="var(--accent,#E8B84B)" }}
                  onMouseLeave={e => { e.currentTarget.style.borderColor="var(--border-default,#2a3444)"; e.currentTarget.style.color="var(--text-muted,#4B5563)" }}
                >
          ...
          <div className="cip-section">
            ...
            <div className="danger-item" onClick={deleteConv}>
              <div className="di-icon" style={{ background:"var(--danger-dim,#ef444415)" }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--danger,#ef4444)" strokeWidth="1.8" strokeLinecap="round">
                  <polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4h6v2"/>
                </svg>
              </div>
              <div>
                <div className="danger-label" style={{ color:"var(--danger,#ef4444)" }}>
...
export default function ConvInfoPage() {
  const router = useRouter()
  return (
    <div style={{ display:"flex", minHeight:"100vh", background:"var(--bg-base,#080C14)", justifyContent:"center" }}>
      <ConvInfoPanel onClose={() => router.back()} />
    </div>
  )
}
